...
export default async function cameraViewerRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.put('/', { preHandler: [requireOrgAdmin] }, async (request, reply) => {
    const params = cameraIdParamsSchema.parse(request.params);
    const body = userIdsBodySchema.parse(request.body);
    const orgId = request.user.org_id!;

    const assigned = await replaceViewersForCamera(
      app.db,
      orgId,
      params.cameraId,
      body.user_ids,
  ...
  app.delete('/', { preHandler: [requireOrgAdmin] }, async (request, reply) => {
    const params = cameraIdParamsSchema.parse(request.params);
    const body = userIdsBodySchema.parse(request.body);
    const orgId = request.user.org_id!;
