...
import {
  getOrgFaceDetectionSettings,
  updateOrgFaceDetectionSettings,
...
export default async function orgFaceDetectionRoutes(app: FastifyInstance): Promise<void> {
  app.get('/', { preHandler: [requireOrgAdmin] }, async (request, reply) => {
    const settings = await getOrgFaceDetectionSettings(app.db, request.user.org_id!);
  ...
  app.patch('/', { preHandler: [requireOrgAdmin] }, async (request, reply) => {
    ...
    const settings = await updateOrgFaceDetectionSettings(app.db, request.user.org_id!, body);
    return reply.code(200).send(settings);
  });
}
